...
| # | Date | Lesson | Source |
|---|------|--------|--------|
| 14 | 2026-08-12 | When the instrumentation cannot see the outcome, rename the outcome — don't estimate it. Web analytics recorded 32 form submits against 4,709 sessions reaching that page, so the funnel's terminal node became "reaches form" instead of "booked". Also: two datasets measuring the same funnel at different population scopes are worth more than either alone — the gap between them *is* a finding (identified-visitor traffic converts 1.6× the all-traffic rate, which quantifies the identification bias). | journey-markov |
...
| 11 | 2026-08-11 | Stepping a simulation per *frame* couples render cost into the physics: anything that makes drawing more expensive (dot size, stroke weight, population) lowers fps and slows the whole world — which presents as the baffling bug "dot size changes movement speed". Advance by wall-clock dt (`acc += rate·dt/16.7ms`, with a stall clamp). Both sims shipped with this; user caught it. | dots + fireflies |
