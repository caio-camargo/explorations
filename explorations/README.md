...
| [`journey-markov/`](journey-markov/) | 2026-08-12 | Journey Gravity — absorbing Markov chain over real (anonymized) site journeys; per-page P(ends booked) — [notes](journey-markov/NOTES.md), [run it](journey-markov/index.html) | v1.3 — 53-visitor ICP cohort; two absorption lenses that invert the ranking (booking event: marketing lane leads; visitor fate: product depth leads). `/enterprise-plan` pinned as the gate — event mode's single green edge. Published with Caio's sign-off 2026-08-12 (aggregate data only) |
